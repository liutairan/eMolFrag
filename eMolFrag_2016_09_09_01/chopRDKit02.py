...
#Main process: 
#1. Use RDKit to make fragments by breaking some bonds and save these fragments to sdf files. Besides dummy atoms and hydrogens, if there are more than 4 atoms in the fragments, the file name will be r-XYZ-00*.sdf; if there are less or equal than 4 atoms in the fragments, the file name will be l-XYZ-00*.sdf.
...
#3. Match the atoms in each rigid to the atoms in mol2 by the coordinates, then get the atom type info. However, some coordinates might not able to match exactly because of round error for float type. So, use the 2-norm of coordinates, the smallest one is the right atom.
...
#chop-rdkit-05.py gives output rigids and linkers with hydrogen
#chop-rdkit-06.py gives output rigids and linkers without hydrogen, exactly as the output format of openbabel scripts.
...
def ChopWithRDKit(outputDir,inputPath):
    ...
    for m in mfl:    
        ...
        if m.GetNumAtoms()-rmAtomCount >=4 :
            tempFileName=output+'r-'+lig+'-'+str(r).zfill(3)+'.sdf'
            r=r+1
        if m.GetNumAtoms()-rmAtomCount <4 :
    ...
    for filePath in fileList:
        ...
        if len(fileName) >0:
            #processing rigid fragments
            if fileName[0] == 'r':
                rigidInfoList=[]
                with open(filePath,'r') as inf:
                    rigidInfoList=inf.readlines()
                #print(rigidInfoList)
                rigidMolEndList=[i for i, x in enumerate(rigidInfoList) if x == '$$$$\n']
                #print(rigidInfoList[:rigidMolEndList[0]])
                fileHead=list(filter(lambda x: 'V2000' in x, rigidInfoList))
                
                fileHeadLineNum=rigidInfoList.index(fileHead[0])
                #print(fileHeadLineNum)
                fileHeadList=fileHead[0].split()
                atomNum=int(fileHead[0][0:3])
                bondNum=int(fileHead[0][3:6])
                atomList=rigidInfoList[fileHeadLineNum+1:fileHeadLineNum+atomNum+1]
                bondList=rigidInfoList[fileHeadLineNum+atomNum+1:fileHeadLineNum+atomNum+bondNum+1]
                ...
                for atomLine in atomList:
                    ...
                    #atom in rigid.sdf, xyz coordinates of one line
                ...
                newRigidInfoList=rigidInfoList[:rigidMolEndList[0]]
                ...
                newRigidInfoList[fileHeadLineNum]=newHead
                newRigidInfoList[0]=fileName+'\n'

                #edit output list
                #edit appendix I - ATOM TYPES
                newRigidInfoList.append('\n')
                newRigidInfoList.append('> <ATOMTYPES> \n')
                newRigidInfoList=newRigidInfoList+atomTypeList[:newAtomNum]
                #edit appendix II - BRANCH ATOM NUMBER AND ELIGIBLE ATMTYPE TO CONNECT
                newRigidInfoList.append('\n')
                newRigidInfoList.append('> <BRANCH @atom-number eligible-atmtype-to-connect> \n')
                newRigidInfoList=newRigidInfoList+newBranchCon
                newRigidInfoList.append('\n')
                newRigidInfoList.append('$$$$\n')

                #remove dummy atoms
                for dummyLine in dummyAtomLineList:
                    newRigidInfoList.remove(dummyLine)
                #remove dummy bonds
                for dummyCon in dummyConnection:
                    newRigidInfoList.remove(dummyCon)
                #remove hydrogen atoms
                for hydrLine in hydrAtomLineList:
                    newRigidInfoList.remove(hydrLine)
                #remove hydrogen bonds
                for hydrCon in hydrConnection:
                    newRigidInfoList.remove(hydrCon)

                #remove M ISO line
                fileMISO=list(filter(lambda x: 'M  ISO' in x, newRigidInfoList))
                #print(fileMISO)
                if len(fileMISO)>0:
                    for ISO in fileMISO:
                        newRigidInfoList.remove(ISO)

                #remove M CHG line
                fileMCHG=list(filter(lambda x: 'M  CHG' in x, newRigidInfoList))
                if len(fileMCHG)>0:
                    for CHG in fileMCHG:
                        newRigidInfoList.remove(CHG)

                #write rigid info to file
                with open(filePath,'w') as outf:
                    outf.writelines(newRigidInfoList)
